...
def wink(frame, f) :
    ...
    # The mouth detection
    mleft_x = mouth_left[0]
    mleft_y = mouth_left[1]
    mleft_pixel = np.array(frame[mleft_y, mleft_x]).astype(np.float64)

    mright_x = mouth_right[0]
    mright_y = mouth_right[1]
    mright_pixel = np.array(frame[mright_y, mright_x]).astype(np.float64)

    mouth_diff = np.linalg.norm(mright_pixel - mleft_pixel)


    EPSILON = 18.2 
    epsilon = 5               
    if(norm_diff > EPSILON and mouth_diff>epsilon) :
        return(True)
    else:
        return(False)
